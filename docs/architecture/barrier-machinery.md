...
| Journal restore | `AggregationExecutor.restore_from_journal` (plus `restore_batch`); validation/hydration in `engine/journal_restore.py` — `AggregationJournalRestorer` | `CoalesceExecutor.restore_from_journal`; validation/hydration in the same module — `CoalesceJournalRestorer` |
