# RC-5.3 Public-Sector Assessment Mapping
...
This document maps the current ELSPETH RC-5.3 evidence set against the public
...
ELSPETH does not claim formal conformance to those frameworks in RC-5.3. This
document identifies the evidence already present and the gaps an adopting agency
would need to close.

## Current Evidence Map

| Area | RC-5.3 evidence present | Gap or caveat |
|---|---|---|
...
| Software artifacts | Build workflow is configured for multi-platform images with SBOM, provenance, and cosign signing; image publication is now gated on the release's required checks passing. | Final RC-5.3 release artifacts, immutable digest, public verification commands, and evidence bundle must be attached to the release record before claiming publication. |
...
| Authentication and identity | RC-5.3 supports local, OIDC, and Microsoft Entra authentication with principal recording claims in guarantees. | Organisation-level authorisation policy remains a deployer responsibility. |
...
- Decide whether a formal DSS/ISM/Essential Eight/PSPF/AGDS/WCAG mapping is in
  scope for RC-5.3 or explicitly deferred.
