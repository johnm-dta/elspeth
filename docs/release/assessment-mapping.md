# 0.7.0 Public-Sector Assessment Mapping
...
This document maps the current ELSPETH 0.7.0 evidence set against the public
...
ELSPETH does not claim formal conformance to those frameworks in 0.7.0. This
document identifies the evidence already present and the gaps an adopting agency
would need to close.

## Current Evidence Map

| Area | 0.7.0 evidence present | Gap or caveat |
|---|---|---|
| Clear intent and audience | README, executive summary, changelog, platform architecture, and guarantees document position ELSPETH as a high-assurance pipeline substrate for auditable workflows. | No formal Digital Service Standard checklist has been completed. |
| User and service understanding | The Composer guide, platform architecture overview, Playwright tests, and executive summary describe target operator and knowledge-worker audiences. | No independent user-research dossier or AGDS assessment is included in the release bundle. |
...
| Software artifacts | Build workflow is configured for multi-platform images with SBOM, provenance, and cosign signing; image publication is now gated on the release's required checks passing. | Final 0.7.0 release artifacts, immutable digest, public verification commands, and evidence bundle must be attached to the release record before claiming publication. |
...
| Authentication and identity | 0.7.0 supports local, OIDC, and Microsoft Entra authentication with principal recording claims in guarantees; local deployments also gain an email-verified registration mode. | Organisation-level authorisation policy remains a deployer responsibility. |
...
| AI and prompt-injection concerns | Composer redaction, tool-call persistence, LLM audit records, LLM-primary guided-mode constraints, pending-interpretation review, prompt-shield review, and advisor sign-off are documented. | A formal AI-specific threat model mapped to current agency controls has not been packaged as a release artifact. |
...
- Decide whether a formal DSS/ISM/Essential Eight/PSPF/AGDS/WCAG mapping is in
  scope for 0.7.0 or explicitly deferred.
