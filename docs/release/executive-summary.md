...
- a commercial offer — ELSPETH is open-source software, developed and publicly released by DTA staff;
- a certified assurance statement — no independent third-party assessment has yet been completed (see *Assurance posture* below).
...
| **Maturity** | Pre-production. 128 days of development to date. The platform has not been deployed in any Australian government agency, and no independent third-party assurance assessment has been completed. |
...
- **Web authoring interface (Composer).** A chat-based authoring surface intended for non-engineering operators to construct pipelines. The interface uses the Australian Government Design System (AGDS) conventions. Accessibility features include skip-to-content navigation, support for reduced-motion preferences, and screen-reader-accessible status indicators.
...
2. **Absence of independent assurance assessment.** The audit-integrity, trust-tier, and access-control claims have been tested internally. They have not been independently assessed by an IRAP-registered assessor or equivalent. An agency adopting the platform under a high-assurance obligation should factor an independent assessment into its adoption plan.
3. **Pre-production maturity.** The platform has not been deployed in production by any agency. Operational characteristics under sustained agency load — concurrent users, audit-database growth over time, and long-running pipelines on contended infrastructure — have been exercised in simulation but not in deployment.
...
- the scope, timing, and cost of an independent assurance assessment that the project would support;
- the shape of an agency-scale pilot, including the proposed use case, the participating agency, the data involved, and the assurance threshold to be met;
...
- the specific open-source licence under which the platform is released, and any associated intellectual-property considerations relevant to an agency's adoption.
