...
## Current Cutover: 0.7.1 Composer parity (session epoch 30 and Landscape epoch 28)
...
an epoch-29 database cannot represent the terminal failure code and must be
recreated. The universal web
...
session epoch is a recreate boundary, including epoch 29. Landscape epoch 28
...
unsupported: keep the service drained, repair the epoch-30 release forward,
recreate fresh state, and retry. Plans 10 and 12 must cite the
session-epoch-30/Landscape-epoch-28 record when binding candidate and rollback
decisions.
...
```bash
sqlite3 "$DB_PATH" 'PRAGMA user_version;'  # expect 30 (== SESSION_SCHEMA_EPOCH)
```

An epoch-29 result is not repairable in place: keep the service drained,
