...
3. **Seam grep — BOTH forms.** Before deleting any method, grep `tests/` for its name in BOTH class-level form (`Orchestrator._x`, `monkeypatch.setattr(Orchestrator, "_x"`) AND instance-level form (`patch.object(orch, "_x"`). Phase 1 found the latter missed by the inventory (3× `patch.object(orch, "_emit_telemetry")` in `test_resume_failure.py`). Repoint every hit to the new owner.
