...
- [ ] Write failing test: sqlite `:memory:`, no tables → `probe_session_schema(engine) is SchemaState.MISSING`; `create_all` minus a table → `PARTIAL`; full+correct → `CURRENT`; full-but-wrong-shape → `STALE`. Same four cases for `probe_landscape_schema`. Run `pytest tests/unit/web/test_schema_probe.py`; expect `ModuleNotFoundError`.
      ...
      with engine.connect() as conn:
          ...
          try:
              ...
              if locked:
                  ...
                  conn.commit()
  ```
  (`pg_advisory_xact_lock` considered instead — spec names `pg_advisory_lock` explicitly, and Task 5's lock-release test below only makes sense against a manual unlock path, so kept as-is.)
  `init_session_schema(engine)`: `_body(conn)` closes over `engine` (probing is Engine-typed — `probe_session_schema` opens its own connection; passing the locked `Connection` in would `AttributeError`, SQLAlchemy 2.0's `Connection` has no `.connect()`) and reserves `conn` for `create_all` only: `state = probe_session_schema(engine)`; `STALE` → `raise SessionSchemaError` with the spec's drop/recreate text verbatim ("drop or recreate the affected Aurora database/schema..., then rerun `elspeth doctor aws-ecs --init-schema`" — not just "run `--init-schema`"); `CURRENT` → return; else (`MISSING`/`PARTIAL`, additively completable per binding semantics) → `session_metadata.create_all(bind=conn, checkfirst=True)`. Call `_run_locked(engine, target="session_schema", body=_body)`, then `if engine.dialect.name == "sqlite": _stamp_schema_sentinels(engine)` — after `_run_locked` commits, since `_stamp_schema_sentinels` opens its own connection and SQLite allows only one writer (idempotent; unreached on STALE). `init_landscape_schema` mirrors this (target=`"landscape_schema"`, `landscape_metadata.create_all`, raise `SchemaCompatibilityError` with the same spec wording, no stamp). Its create branch omits `_create_additive_indexes()`: sole member `ix_tokens_run_id` is a normally-registered `Index` (`landscape/schema.py:1078`), so `create_all` already creates it here — the additive-indexes method is for backfilling a pre-existing table, out of scope (no SQLite-to-Aurora migration).
