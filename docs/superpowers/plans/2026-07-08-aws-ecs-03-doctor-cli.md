...
def probe_writable(label: str, path: Path) -> ContractCheck:
...
def collect_checks(settings: WebSettings) -> list[ContractCheck]:
    ...
    checks.append(probe_writable("data_dir", settings.data_dir))
...
def _probe_and_check_schema(label, raw_url, probe_fn, init_fn, *, init_schema: bool) -> ContractCheck:
    ...
    if init_schema and state in (SchemaState.MISSING, SchemaState.PARTIAL):
        try:
            ...
            # init_fn raises exactly these two types on exactly one path
            # (plan 02 Task 3): its own advisory-locked re-probe found STALE,
            # a race this function's outer probe_fn(engine) couldn't have
            # seen. Surface schema_check's STALE detail (the drop/recreate
            # operator instructions) rather than a sanitized class name that
            # would discard them.
...
- [ ] Write `test_doctor_init_schema_succeeds_against_fresh_postgres` (`pytest.mark.testcontainer`): two fresh `PostgresContainer("postgres:16-alpine", driver="psycopg")`; build `WebSettings` with both URLs pointed at the containers, `data_dir`/`payload_store_path` under `tmp_path`, production-shaped `secret_key`/`shareable_link_signing_key`, `host="0.0.0.0"`; `collect_checks(settings, init_schema=True)` → `all(c.ok for c in checks)`.
