...
**Depends on:** `2026-07-08-aws-ecs-08-s3-endpoint-gate.md` — `aws_s3` must not reach a web-catalog-reachable deployment before plan 08's `endpoint_url` gate lands (auto-discovery registers it into the web composer catalog on merge, so registration = web-reachable). This plan's tasks have no other dependency and may be built in parallel; the release/deploy checklist must not let this reach staging or prod ahead of plan 08. **Round 3 — the ordering is now mechanical, not checklist-enforced:** Task 2's registration commit includes `test_registered_aws_s3_source_is_endpoint_url_gated`, which drives `validate_pipeline` and is deliberately **red on any tree carrying this registration without plan 08 Tasks 1–2** — merging this plan ahead of the gate cannot pass CI.
...
- [ ] Write `test_registered_aws_s3_source_is_endpoint_url_gated` in `tests/unit/plugins/sources/test_aws_s3_source.py` (round 3 — the merge-order guard that makes the overview's "gate lands before registration" ordering structural instead of prose): build a minimal `CompositionState` whose source is `SourceSpec(plugin="aws_s3", options={"bucket": "b", "key": "k.csv", "endpoint_url": "https://attacker.example", "schema": {"mode": "observed"}})` (mirror the arrange block of plan 08 Task 2's `TestValidatePipelineAwsS3EndpointUrlPolicy`), run `validate_pipeline(...)`, and assert `result.is_valid is False` with a failed `aws_s3_endpoint_url_policy` check. **This test is deliberately red on any tree that carries the `aws_s3` registration without plan 08 Tasks 1–2** — that red is its purpose; do not skip or xfail it to "unblock" a premature merge. Run it → PASS (plan 08 Tasks 1–2 merge first per the overview's within-wave order).
