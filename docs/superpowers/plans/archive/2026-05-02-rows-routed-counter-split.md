...
8. No occurrence of the retired aggregate/event field names remains in the source tree, tests, or frontend/stable docs **as a code reference**. The gate's intent is "no stale code idioms naming the legacy field" — kwargs (`rows_routed=`), attribute access (`.rows_routed`), type annotations (`rows_routed: int`), dict subscripts (`obj["rows_routed"]`), and function-arg references (`(rows_routed`). Documentation prose that *describes* the rename (docstrings, inline comments, ADR/runbook text, test docstrings citing the historical name) is **intentional and exempt** — the rename's discoverability depends on those references. Verify with THREE commands:
   - Python sources/tests, `rows_routed` code idioms: `grep -rnE '\brows_routed\s*=|\.rows_routed\b|\brows_routed\s*:|\["rows_routed"\]' src/elspeth/ tests/ --include="*.py"` — expected zero matches. (NOTE: this is a tightened pattern. The original `\brows_routed\b` regex is too broad — it catches every prose mention in docstrings and comments. The tightened pattern matches kwargs (`rows_routed=`), attribute access (`.rows_routed`), type annotations (`rows_routed:`), and dict subscripts (`["rows_routed"]`). Earlier drafts also included `\(rows_routed\b`, but that pattern produces false positives on parenthetical prose like `(rows_routed counter split)` inside docstrings; it's redundant because `\brows_routed\s*=` already catches kwargs in function calls. If you want to re-survey *all* prose mentions for accuracy, run `grep -rn "rows_routed\b" src/elspeth/ tests/ --include="*.py"` and inspect — every hit should be in a docstring, comment, or test docstring describing the rename. The one stability test at `tests/unit/web/execution/test_schemas.py:1305` asserts `"rows_routed" not in properties` and is intentional — it pins absence of the legacy field in the public API surface.)
   - Python sources/tests, bare `RunSummary`-style `.routed` consumer or `routed=` producer: `grep -rn "RunSummary[^)]*routed[^_]\|\.routed\b\|\brouted=" src/elspeth/ tests/ --include="*.py" | grep -v "routed_success\|routed_failure\|routed_destinations\|RowOutcome\.ROUTED\|RoutingMode\|RoutingEvent\|routed (sink\|routed_summary\|routed rows\|routed via\|routed to "` — expected zero matches. Pins the RunSummary-level split and catches any missed `event.routed` consumer or `routed=` producer call site. (This gate is already pattern-tightened with explicit allowlist via `grep -v` — code-idiom-precise as written.)
   - Frontend + stable docs (code idioms only): `grep -rnE '\brows_routed\s*=|\.rows_routed\b|\brows_routed\s*:|\browsRouted\s*[=:]|\.rowsRouted\b' src/elspeth/web/frontend/src/ docs/architecture/ docs/contracts/ docs/guides/ docs/runbooks/ docs/reference/ | grep -v "adr/004-adr-explicit-sink-routing\|adr/018-producer-site-outcome-discrimination\|runbooks/database-maintenance"` — expected zero matches. The three excluded files (`adr/004-adr-explicit-sink-routing.md`, `adr/018-producer-site-outcome-discrimination.md`, `runbooks/database-maintenance.md`) each contain markdown-backticked references to the legacy `ExecutionCounters.rows_routed` identifier as part of *documenting* the rename — those references are intentional and exempt; the regex catches them as attribute-access syntax even though they're prose. The frontend is at `src/elspeth/web/frontend/src/` (NOT `frontend/`, `web/`, or `src/elspeth/web/static/` — those paths do not exist; pre-fix verification 2026-05-02 returned zero hits there). The docs target list is **deliberately narrow**: only published / stable contract subdirs (architecture ADRs, token-outcome contracts, user guides, runbooks, reference docs) are gated. The following docs subdirs are EXCLUDED from the gate because they legitimately discuss the old name in prose (specs in flight, audits, bug write-ups, analysis dumps, archived plans): `docs/superpowers/plans/` (this plan and any sibling spec drafts), `docs/superpowers/specs/`, `docs/audits/`, `docs/bugs/`, `docs/analysis/`, `docs/arch-analysis-*/`, `docs/arch-pack-*/`, `docs/release/`, `docs/requirements/`, `docs/assets/`. Greppping all of `docs/` would self-fail because the active plan file itself contains 150+ legitimate prose references to `rows_routed` (task descriptions, replacement instructions, terminology section, etc.). Note: ADR-018, the ADR-004 amendment section, and the database-maintenance runbook each contain prose references to `rows_routed` because they *document* the rename — those references are intentional (the ADR's CONTEXT section names what was renamed; the runbook's operator instructions name the pre-split state operators must clean up). The tightened regex above does not flag prose, so these legitimate references no longer trip the gate.
...
```bash
# NOTE: tightened from the original `rows_routed\b|\.routed\b|routed=` to a
# code-idiom-only pattern that does not catch docstring/comment prose (e.g.,
# `Post elspeth-5069612f3c (rows_routed counter split):` inside a test
# docstring is intentional documentation of the rename and not a stale code
# reference). The original wording produced false positives on every test file
# whose docstrings legitimately reference the historical name. The tightened
# pattern catches the actual stale-code idioms — kwargs (`rows_routed=`),
# attribute access (`.rows_routed`), type annotations (`rows_routed: int`),
# dict subscripts (`["rows_routed"]`), and the bare RunSummary-style
# `.routed`/`routed=` patterns.
for f in tests/unit/contracts/test_enums.py tests/unit/contracts/test_events.py tests/unit/web/sessions/test_models.py tests/unit/web/sessions/test_schema.py tests/unit/web/sessions/test_routes.py tests/unit/web/sessions/test_service.py tests/unit/web/execution/test_websocket.py tests/unit/web/sessions/test_protocol.py tests/unit/web/execution/test_routes.py tests/unit/web/execution/test_service.py tests/unit/web/execution/test_schemas.py tests/integration/cli/test_cli.py tests/property/engine/test_orchestrator_lifecycle_properties.py; do
  hits=$(grep -cE '\brows_routed\s*=|\.rows_routed\b|\brows_routed\s*:|\["rows_routed"\]|\.routed\b|\brouted\s*=' "$f" 2>/dev/null || echo 0)
  [ "$hits" -gt "0" ] && grep -nE '\brows_routed\s*=|\.rows_routed\b|\brows_routed\s*:|\["rows_routed"\]|\.routed\b|\brouted\s*=' "$f" | grep -v "routed_success\|routed_failure\|routed_destinations\|routed (sink\|routed_summary\|routed rows\|routed via\|routed to " | sed "s|^|$f:|"
done
```
