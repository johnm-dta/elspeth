...
|---|---|---|---|
| Per-ref content size (upper) | TBD pending VAL data — measured against p99 byte length of existing inline `system_prompt` / `query` / `template` field content sampled from the audit DB | Unit + integration coverage of `BlobContentResolutionError(oversized=...)`; numeric value lands in the ADR with cited dataset | M1 |
| Per-ref content size (lower) | TBD pending VAL data — refs for fields under N bytes (working candidate: 256B) discouraged via warning to prevent "everything is a ref" anti-pattern | Same harness as upper bound | M1 |
| Aggregate per-config bytes cap | TBD pending VAL data — bounds total resolved-content bytes per config tree to keep preflight latency within the existing 30-second `_call_async` timeout budget for an N-ref config | Unit test pinning the aggregate-budget arithmetic; integration test with N refs at the per-ref cap demonstrating preflight completes within 30s on CI infra | M1, S2 |
...
The "TBD pending VAL data" cells are deliberate: per the panel's M1 finding (4 of 5 reviewers), unprincipled caps get re-litigated every time someone hits them. Closing them now without dataset evidence ships the same bug the spec exists to close. The numbers land in the ADR (P1) with cited dataset.
