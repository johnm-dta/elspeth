...
| `preview_pipeline` (dry-run) | `composer/tools/generation.py:1685` (`_execute_preview_pipeline`) | Returns validation, edge/semantic contracts, proof diagnostics, node/output overview **without executing**. Feeds the graph's annotations **and** acts as the instant pre-flight gate ("is the structure sound before we ever run?"). |
...
- **5.1 Layout.** Primary chat docked bottom + a persistent live-graph panel in the guided column (mount `GraphView` or `GraphMiniView`+`GraphModal`, reading the existing store). The graph replaces the "current decision = LLM prose" review block as the canonical "what I built" surface.
...
- **5.6 Kill vestigial vocabulary.** `step_2_5_recipe_match` / `recipe_offer` (no session reaches them), `coaching` / `recipe_match` profile flags (plumbed to wire, never read), `chosen:["reject"]`→501, `inspect_and_confirm` (test-only), `interpretation_review`→null dispatch. Removing dead branches shrinks the surface the reframe has to reason about. (Cross-check the `SlotType / guided.ts mirror consistency` pre-commit hook on any enum edit.)
