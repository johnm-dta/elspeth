...
#   2. Creates a NEW session, loads the YAML (uses /set_pipeline if available,
#      else uploads via the YAML import endpoint).
...
#
# Exit codes:
#   0 ok / 64 bad usage / 67 missing file / 70 login / 76 import-yaml failed
...
# The composer exposes either POST /api/sessions/{sid}/state/yaml or
# POST /api/sessions/{sid}/import-yaml depending on version. Try both.
...
if [[ "$http" != 2* ]]; then
  evals_log WARN "POST /state/yaml returned $http; trying /import-yaml"
  http=$(_evals_http_post_json \
           "$ELSPETH_EVAL_BASE_URL/api/sessions/$sid/import-yaml" \
           "$import_body" \
           "$import_resp" || echo 000)
fi
if [[ "$http" != 2* ]]; then
  evals_die 76 "YAML import failed (last HTTP $http): $(head -c 500 "$import_resp" 2>/dev/null)"
fi
