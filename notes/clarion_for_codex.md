...
Check whether Codex already knows about the server:

```bash
codex mcp list
```

The repository also carries a working example in `.vscode/.mcp.json`, but do
not assume the Codex CLI has loaded that file. `codex mcp list` is the live
check.
