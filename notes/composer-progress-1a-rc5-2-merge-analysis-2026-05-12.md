...
## 11. Rev-3 amendment — reality re-verification against post-guided-mode RC5.2 (2026-05-13)

**Trigger:** RC5.2 advanced from `3e46a976` (rev-2 reference) to `ff46b809` (Merge PR #37: composer guided-mode wizard Phases 1-10 + PR-review fixes) and a follow-on doc commit `a7a4a640`. The +117-commit advance on RC5.2 invalidates several rev-2 claims. A pre-task analysis session attempted a rev-3 framing (the "rev-3 dispatch" hypothesised below) but was itself working from a stale mental model. This §11 supersedes both rev-2 and that dispatch.

All claims in this section are verified against `git show <ref>:<path>` reads (not working-tree greps — see §11.7 below for why this matters).

### 11.1 Reframing — Phase 1A already on RC5.2 (rev-4 form)

Commit `11fc0ce4d` ("composer-progress-persistence phase 1, rev-4 add_message rewrite — composer-progress-persistence phase 1") landed on RC5.2 via the guided-mode PR #37 merge. The commit body declares:

> BREAKING: SessionServiceProtocol.add_message and SessionServiceImpl.add_message now require a keyword-only `writer_principal` argument matching the `ck_chat_messages_writer_principal` CHECK enum. Adds optional `tool_call_id` and `parent_assistant_id` arguments (required when role='tool', forbidden otherwise — enforced by CHECK constraints). Adds `sequence_no` allocation under _session_write_lock.

That is **Phase 1A in rev-4 form** — newer than the rev ≤3 form on `feat/composer-progress-persistence-1a`. The schema currently on RC5.2 includes:

- `composition_states.provenance String NOT NULL` with the 6-value CHECK constraint `provenance IN ('tool_call', 'convergence_persist', 'plugin_crash_persist', 'preflight_persist', 'session_seed', 'session_fork')`.
- `chat_messages.tool_call_id`, `chat_messages.sequence_no`, `chat_messages.writer_principal`, `chat_messages.parent_assistant_id`.

`composer-progress-1a`'s older Phase 1A version coexists at a different revision level. The merge must adopt RC5.2's rev-4 as canonical and supersede the branch's older form. The dominant rev-3 reconciliation work is therefore **column-position alignment in `sessions/models.py`**, not "bring Phase 1A forward" as rev-2 framed it.

### 11.2 Provenance threading — already wired on RC5.2

Rev-2 §6.1 catalogued Phase 1A as including `composition_states.provenance` and implicitly assumed the schema was unique to `composer-progress-1a`. The pre-task rev-3 dispatch sharpened this into a specific claim:

> guided-mode-new call sites... NOT auto-resolved. All 6 need manual provenance threading post-merge.

**This claim is disproven.** Verified via `grep -nA8 "service.save_composition_state(" src/elspeth/web/sessions/routes.py` against RC5.2 HEAD:

| Line | Call site context | `provenance=` already wired? |
|---|---|---|
| 1227 | partial_record on tool-call path | `provenance="convergence_persist"` ✓ |
| 1367 | partial_record on plugin-crash path | `provenance="plugin_crash_persist"` ✓ |
| 1596 | partial_record on preflight-failure path | `provenance="preflight_persist"` ✓ |
| 2252 | new_state_record (guided new state transition) | wired ✓ |
| 2723 | new_state_record (terminal state_record_out) | `provenance="session_seed"` ✓ |
| 3213 | copied_state on fork_session path | wired ✓ |

All 6 production callers already pass a valid `provenance=` value. The "6 new sites needing provenance threading post-merge" concern is moot. The pre-task rev-3 cost line item for this is removed.

### 11.3 Replacement-shape — `2c043a11` deletion holds on RC5.2

Verified via `git show RC5.2:src/elspeth/web/composer/service.py | grep -c "_runtime_preflight_failure_message\|_enforce_replacement_non_prefix_invariant\|_ReplacementBranch\|_INTERCEPTED_ASSISTANT_HISTORY_PREFIX"`: **0 matches**.

The deletion `2c043a11` performed is intact through the guided-mode merge and the 30 post-merge fixes. The 12+ call sites still live on `composer-progress-1a` (per §5.2) are unchanged from rev-2. The reconciliation work in §9 step 3 (delete 4 symbols, ~5 call sites, 7 tests) remains valid for the branch side.

A naive working-tree grep against the main checkout may APPEAR to show the symbols still present on RC5.2 — this is a working-tree contamination artefact (see §11.7). The ground truth from `git show RC5.2:` is the authoritative answer: 0 matches.

### 11.4 Conflict set — 9 textual conflicts (rev-2's 5 plus 4 new)

`git merge-tree --write-tree feat/composer-progress-persistence-1a origin/RC5.2` reports:

```
CONFLICT (content): Merge conflict in config/cicd/enforce_tier_model/web.yaml     [rev-2 carry-over]
CONFLICT (content): Merge conflict in src/elspeth/contracts/audit.py              [NEW — guided-mode +3]
CONFLICT (content): Merge conflict in src/elspeth/plugins/transforms/batch_data_quality_report.py  [NEW — guided-mode +5/-1]
CONFLICT (content): Merge conflict in src/elspeth/web/composer/tools.py           [NEW — flipped from auto-merge in rev-2]
CONFLICT (content): Merge conflict in src/elspeth/web/execution/fanout_guard.py   [NEW — guided-mode +7]
CONFLICT (content): Merge conflict in src/elspeth/web/execution/routes.py          [rev-2 carry-over]
CONFLICT (content): Merge conflict in tests/unit/web/sessions/test_models.py       [rev-2 carry-over]
CONFLICT (content): Merge conflict in tests/unit/web/sessions/test_routes.py       [rev-2 carry-over]
CONFLICT (content): Merge conflict in uv.lock                                      [rev-2 carry-over]
```

The most consequential new conflict is `src/elspeth/web/composer/tools.py`: in rev-2 this file auto-merged (composer-progress-1a's Phase 2 promotion wave at L5250-5314 vs RC5.2's 4-commit grounding-detector / augment-shape / state-claim correction set). Guided-mode's additional tool-handler changes now textually collide. Highest-care resolution required (see §11.6).

### 11.5 Unique surface on `composer-progress-1a` (verified via `git show RC5.2:`)

| Surface | On RC5.2? | Evidence |
|---|---|---|
| Phase 1A core schema (provenance + writer_principal + tool_call_id + sequence_no + parent_assistant_id columns + CHECKs) | ✅ Yes (rev-4) | `11fc0ce4d` commit, `git show RC5.2:src/elspeth/web/sessions/models.py \| grep -c "provenance" = >0` |
| Phase 1A `audit_access_log_table` (INERT) | ❌ No | `git show RC5.2:src/elspeth/web/sessions/models.py \| grep -c "audit_access_log_table" = 0` |
| Phase 1B (`SessionsService.persist_compose_turn` + `_persist_payload.py` DTOs) | ❌ No | `git show RC5.2:src/elspeth/web/sessions/_persist_payload.py \| wc -l = 1` (effectively empty stub) |
| Phase 1C testcontainer marker + psycopg2-binary dep | ❌ No | `git show RC5.2:pyproject.toml \| grep -c "testcontainer" = 0` |
| Phase 2 redaction walker (`redaction.py`) | ❌ No | `git show RC5.2:src/elspeth/web/composer/redaction.py \| wc -l = 42` (vs 2752 on branch) |
| Replacement-shape machinery (4 symbols) | ❌ No (deleted by `2c043a11`) | See §11.3 |
| Phase 3 plan rev-2 doc | ❌ No | branch-only file |

The unique surface enumeration is **Phase 1B + Phase 1C + Phase 2 + audit_access_log_table + F1-F6 hardening + Phase 3 plan doc**. Phase 1A is the only piece that REQUIRES reconciliation (because both sides have a version); the rest is additive on the branch side.

### 11.6 Strategy decision — Option A (merge with rev-4 reconciliation), operator-chosen 2026-05-13

The rev-2 doc framed three options. Given the rev-3 evidence, the operator chose **Option A: real merge with rev-4 reconciliation** (2026-05-13 session). The execution plan lives at `/home/john/.claude/plans/please-plan-this-merge-eager-meerkat.md` and breaks down as:

- Step 0: dedicated merge worktree at `.worktrees/rc5-2-merge-from-1a` with worktree-local venv (per `feedback_uv_venv_leak`)
- Step 1: this §11 amendment
- Step 2: pre-merge reconciliation on `composer-progress-1a` — (A) adopt rev-4 Phase 1A column layout, (B) delete replacement-shape per `2c043a11`, (C) full gate suite green
- Step 3: execute merge into the merge worktree
- Step 4: resolve 9 textual conflicts (mechanical → semantic order; `composer/tools.py` last)
...
- Step 8: re-baseline Phase 3 work — rebase `composer-progress-1a` onto post-merge RC5.2 OR fresh `feat/composer-progress-persistence-phase3` branch (preference α: rebase, per `project_adr010_umbrella_branch` umbrella convention)

Updated cost estimate: ~1.5 days end-to-end (lower than the pre-task dispatch's 1.5-2 days because provenance threading is removed from the work list).

### 11.7 Methodology caveat — working-tree contamination

During the rev-3 verification session, an initial `git grep` against the main checkout reported `redact_tool_call_arguments` at line 1658 of `redaction.py` and replacement-shape symbols still live on RC5.2. Both were artefacts: the main checkout's working tree had a stale copy of `redaction.py` (2752 lines) inherited from a prior worktree-cross-contamination event; `git status` showed an `M` flag on `routes.py` that `git diff` reported as zero-content (stat-only).

`git update-index --refresh` cleared the stat-only `M`. The stale working-tree content was bypassed by reading via `git show <ref>:<path>` directly. Every claim in this §11 is verified against an explicit `git show RC5.2:<path>` read, not against working-tree content.

**Methodology rule for future revisions:** when multiple worktrees exist for the same repo, never trust working-tree grep results unless the cwd is freshly synchronised. Prefer `git show <ref>:<path> | grep -c ...` for verification queries that span branches.

### 11.8 Disposition

- §11 supersedes rev-2 §6.1's framing that Phase 1A is exclusive to the branch.
- §11 supersedes the pre-task rev-3 dispatch's claim that 6 guided-mode-new call sites need provenance threading post-merge.
- §5.1–§5.4 (rev-2's replacement-shape analysis) **remains load-bearing** — `2c043a11` deletion holds on RC5.2; the branch must align by deleting the symbols and 7 pinning tests.
- §9 step 3 (the 7-test deletion list) **remains load-bearing**.
- The cost table in rev-2 §1 ("half-day textual + full gate rerun") is approximately correct, but the pre-merge reconciliation work is dominated by rev-4 Phase 1A schema alignment rather than the bring-Phase-1A-forward interpretation.

**Operator action at rev-3:** execute per the merge plan. If discrepancies surface during execution, fold corrections back as rev-4. Critical surfaces to re-verify before each substep: Step 2A diff inspection (column-position only vs semantic divergence), Step 4 conflict 9 resolution (`composer/tools.py` L5250-5314 region), Step 5 adequacy guard (`test_adequacy_guard.py` manifest drift).
