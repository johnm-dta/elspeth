...
#  11 → Multi-source foundation: per-source run records, source-scoped row
#        indexes, global ingest sequence ordering, and durable token work items.
#  12 → Durable scheduler continuation routing: token_work_items.on_success_sink
#        preserves sink-bound continuations across resume/recovery.
#  13 → Durable scheduler resume identity: token_work_items stores token lineage
#        and coalesce cursor fields so resumed workers do not depend on in-memory
#        pending_items state.
SQLITE_SCHEMA_EPOCH = 13
