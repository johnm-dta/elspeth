...
- `path_a.input: "path_a_in"`, `path_b.input: "path_b_in"` — consume the gate's published connections (must match `fork_to` entries exactly).
- `path_a.on_success: "path_a_out"`, `path_b.on_success: "path_b_out"` — publish downstream branch connections after processing.
- `coalesce.branches: ["path_a_out", "path_b_out"]` — consume the path-transform outputs (must match the path nodes' `on_success` values exactly).

`fork_to` and `coalesce.branches` are **two different connection-name lists**: `_in` names connect gate→path-transform, `_out` names connect path-transform→coalesce. Using node IDs in either list, or reusing the same name on both sides of a path node, is the canonical wiring failure. The worked example below applies this discipline literally — read the connection names side-by-side before generating your own.
