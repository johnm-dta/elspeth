...
def _coerce_source_mapping(sources: Mapping[str, SourceSpec] | SourceSpec | None) -> Mapping[str, SourceSpec]:
    """Return canonical source mapping for helpers that still support legacy callers."""
    if sources is None:
        return {}
    if isinstance(sources, SourceSpec):
        return {"source": sources}
    return sources


def _batch_distribution_profile_value_field_entries(
    sources: Mapping[str, SourceSpec] | SourceSpec | None,
