...
Plane-by-plane decomposition in progress. Extracted so far:

- ``_common`` — ``ToolResult``, response helpers, repair-suggestion synthesis,
  base serialisation. The shared toolkit every plane imports from.
- ``blobs`` — session-scoped binary blob storage handlers, quota state,
  storage primitives, blob DTOs and signal exceptions, blob-only frozensets.

Remaining material in ``_legacy`` is pending its own per-plane slice.
"""
...
from elspeth.web.composer.tools._legacy import (  # noqa: F401
