...
currently lives in ``_legacy``; this ``__init__`` re-exports every top-level symbol
so the public path ``elspeth.web.composer.tools.<symbol>`` keeps working unchanged
across the split. Re-exports include underscore-prefixed internals because tests
import them directly and ``import *`` does not re-export underscored names.
"""
