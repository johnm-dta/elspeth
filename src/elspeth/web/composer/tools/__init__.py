...
This package is being decomposed plane-by-plane. The consolidated implementation
currently lives in ``_legacy``; the shared toolkit (``ToolResult``, response
helpers, repair-suggestion synthesis, base serialisation) has been extracted to
``_common``. This ``__init__`` re-exports every symbol so the public path
``elspeth.web.composer.tools.<symbol>`` keeps working unchanged across the split.
"""
