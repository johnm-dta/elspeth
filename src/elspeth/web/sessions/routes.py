...
def _extract_runtime_model_snapshot(
    ...
    Mapping shape is guaranteed; ``id`` is a structurally required key
    (Tier-1 read — KeyError on absence is correct behaviour). ``model``
    and ``model_version`` are *optional* keys by design — an LLM transform
    without an explicit pin uses an LLM-pack default at runtime. The
    audit row's columns are nullable; recording ``None`` accurately
    ...
    for node in state.nodes:
        ...
        version = node.get("model_version")
        if identifier is not None and type(identifier) is not str:
            raise AuditIntegrityError(
                f"Tier 1 audit anomaly: node {node_id!r}.model is "
        ...
        if version is not None and type(version) is not str:
            raise AuditIntegrityError(
                f"Tier 1 audit anomaly: node {node_id!r}.model_version is "
...
def create_session_router() -> APIRouter:
    ...
    # ------------------------------------------------------------------ #
    # Interpretation events (Phase 5b Tasks 6 + 7)
    # ------------------------------------------------------------------ #
    #
    # DI / telemetry note:
    ...
    async def resolve_interpretation(
        ...
        """User-driven resolve of a pending interpretation event (Phase 5b Task 6).
        ...
        Status-code mapping for service ``ValueError``: the service
        raises a single ``ValueError`` for three distinct failure modes —
        non-existent event, cross-session reference, and double-resolve
        (TOCTOU).  A follow-up read disambiguates: a row matching
        ``(id, session_id)`` that is NOT pending → 409 (conflict —
        already resolved); else 404 (not found / cross-session — both
        IDOR-safe).
        """
        ...
        pending_events = await service.list_interpretation_events(session_id, status="all")
        ...
        try:
            ...
            # Disambiguate.  If a row exists at ``(id, session_id)`` with
            # ``choice != 'pending'``, this is a double-resolve / TOCTOU
            # collision: return 409.  Otherwise (unknown id, or
            # cross-session reference), return 404 — IDOR-safe.
            existing = next(
                (e for e in pending_events if e.id == event_id and e.choice is not InterpretationChoice.PENDING),
                None,
            )
            if existing is not None:
                raise HTTPException(
                    status_code=409,
                    detail="Interpretation event is already resolved.",
                ) from exc
            raise HTTPException(status_code=404, detail="Interpretation event not found") from exc
    ...
    async def list_interpretations(
        ...
        """List interpretation events for the session (Phase 5b Task 6).
    ...
    async def opt_out_of_interpretations(
        ...
        """Record the per-session 'stop asking about interpretations' decision (Phase 5b Task 7).
