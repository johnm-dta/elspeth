...
This file is a self-contained prompt template. Paste it into a fresh Claude Code session (or send it as the user message), substituting `<<CHUNK_ID>>` with one of the chunk IDs from `tests/audit/CHUNKS.md` (e.g., `U-ENGINE-1`, `I-2`, `P-1`).
...
4. Approve the agent dispatch and file writes. The session will:
   - Enumerate the chunk's file list from `tests/audit/CHUNKS.md`
   ...
   - Update `tests/audit/CHUNKS.md` progress table and `tests/audit/README.md` cross-chunk patterns
...
tests, alongside major coverage gaps. Findings are persisted to `tests/audit/` and
...
1. `tests/audit/CHUNKS.md` — the chunk plan and progress state. Find chunk
   <<CHUNK_ID>> in the tables; note its file scope description.
2. `tests/audit/README.md` — the cross-chunk patterns and methodology section.
   Note any patterns flagged as suite-wide so the new wave can confirm or extend them.
3. `tests/audit/<most-recent-completed-chunk>-findings.md` — read the most recently
...
Once all 5 agents return, write `tests/audit/<chunk>-findings.md` with these sections
...
Then write `tests/audit/<chunk>-raw-reports.md` with the verbatim outputs of all
