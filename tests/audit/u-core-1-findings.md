...
The other findings (deletable test bodies, hash-without-binding sweep, scenario gaps, attributability round-trip, terminal-state coverage) are not yet filed — they are test-quality cleanups or chunk-wide refactors, best handled as sweep PRs rather than per-test issues. The hash-without-binding and `hasattr` patterns are suite-wide and warrant CI rules (recorded in `tests/audit/README.md`).
