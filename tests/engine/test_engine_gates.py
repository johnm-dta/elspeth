...
# ============================================================================
# WP-14b: Plugin Gate + Config Gate Interaction
# ============================================================================


class TestMixedGatePipeline:
    """WP-14b: Verify plugin gates and config gates can coexist in a pipeline.

    ELSPETH supports two types of gates:
    1. Plugin gates - BaseGate subclasses with evaluate() method
    2. Config gates - GateSettings with expression-based conditions

    Both can coexist in the same pipeline. Plugin gates are processed in the
    transforms list, config gates are processed after transforms. This test
    verifies the interaction works correctly.

    Pipeline structure:
        source -> enrichment_transform -> plugin_gate -> config_gate -> sinks

    Plugin gate routes values < 10 to "too_low" sink.
    Config gate routes values > 30 to "high" sink, else "mid" sink.

    Expected flow:
    - Values < 10: routed by plugin_gate to "too_low" (never reach config_gate)
    - Values 10-30: pass plugin_gate, config_gate routes to "mid"
    - Values > 30: pass plugin_gate, config_gate routes to "high"
    """

    def test_plugin_gate_then_config_gate(self) -> None:
        """Pipeline with transform -> plugin_gate -> config_gate -> sinks.

        Verifies:
        - Plugin gate routes some rows to a sink (e.g., "too_low")
        - Config gate routes remaining rows based on expression
        - Both gate types process correctly in sequence
        - Rows have enriched data from transform
        """
        from elspeth.contracts import TransformResult
        from elspeth.core.dag import ExecutionGraph
        from elspeth.core.landscape import LandscapeDB
        ...
        class EnrichTransform(BaseTransform):
            """Transform that doubles value and adds a label."""
                ...
                return TransformResult.success(
                    {
                        "value": val,
                        "doubled": val * 2,
                        "label": f"processed_{val}",
                    }
                )

        # === Plugin Gate (routes values < 10 to "too_low") ===
        class ThresholdGate(BaseGate):
            """Gate that routes low values to 'too_low' sink."""
            ...
            def evaluate(self, row: dict[str, Any], ctx: Any) -> GateResult:
                if row["value"] < 10:
                    # Route to "too_low" sink
                    return GateResult(
                        row=row,
                        action=RoutingAction.route(
                            "below",  # Route label that maps to "too_low" sink
                            reason={"value": row["value"], "threshold": 10},
                        ),
                    )
                # Continue to next stage (config gate)
                return GateResult(
                    row=row,
                    action=RoutingAction.continue_(
                        reason={"value": row["value"], "passed": True}
                    ),
                )
        ...
        # === Setup test data ===
        # Values covering all routing paths:
        # - 5: < 10, routed by plugin_gate to "too_low"
        # - 8: < 10, routed by plugin_gate to "too_low"
        # - 15: >= 10 and <= 30, passes plugin_gate, config_gate routes to "mid"
        # - 25: >= 10 and <= 30, passes plugin_gate, config_gate routes to "mid"
        # - 35: >= 10 and > 30, passes plugin_gate, config_gate routes to "high"
        # - 50: >= 10 and > 30, passes plugin_gate, config_gate routes to "high"
        source = ListSource(
            [
                {"value": 5},
                {"value": 8},
        ...
        # Plugins
        enrich_transform = EnrichTransform(config={})
        threshold_gate = ThresholdGate(config={})

        # Sinks
        too_low_sink = CollectSink()
        mid_sink = CollectSink()
        high_sink = CollectSink()

        # Config gate: routes based on value thresholds
        # Values > 30 go to "high", others go to "mid"
        ...
        config = PipelineConfig(
            source=source,
            transforms=[enrich_transform, threshold_gate],  # Plugin gate in transforms
            sinks={"too_low": too_low_sink, "mid": mid_sink, "high": high_sink},
            gates=[config_gate],  # Config gate after transforms
        )

        # === Build execution graph ===
        # The graph structure:
        #   source -> transform_0 (enrich) -> transform_1 (threshold_gate)
        #   -> [plugin gate routes "below" -> sink_too_low]
        #   -> [plugin gate "continue"] -> config_gate_range_router
        #   -> [config gate routes "true" -> sink_high]
        #   -> [config gate routes "false" -> sink_mid]

        graph = ExecutionGraph()

        # Add nodes
        graph.add_node("source", node_type="source", plugin_name="list_source")
        graph.add_node("transform_0", node_type="transform", plugin_name="enrich")
        graph.add_node("transform_1", node_type="gate", plugin_name="threshold_gate")
        graph.add_node(
            "config_gate_range_router",
            node_type="gate",
            plugin_name="config_gate:range_router",
            config={
                "condition": config_gate.condition,
                "routes": dict(config_gate.routes),
            },
        )
        graph.add_node("sink_too_low", node_type="sink", plugin_name="collect")
        graph.add_node("sink_mid", node_type="sink", plugin_name="collect")
        graph.add_node("sink_high", node_type="sink", plugin_name="collect")

        # Add edges
        # source -> enrich transform
        graph.add_edge("source", "transform_0", label="continue", mode=RoutingMode.MOVE)
        # enrich transform -> plugin gate
        graph.add_edge(
            "transform_0", "transform_1", label="continue", mode=RoutingMode.MOVE
        )
        # plugin gate -> too_low sink (for "below" route)
        graph.add_edge(
            "transform_1", "sink_too_low", label="below", mode=RoutingMode.MOVE
        )
        # plugin gate -> config gate (for "continue" route)
        graph.add_edge(
            "transform_1",
            "config_gate_range_router",
            label="continue",
        ...
        # config gate -> high sink (for "true" route)
        graph.add_edge(
            "config_gate_range_router",
            "sink_high",
            label="true",
        ...
        # config gate -> mid sink (for "false" route)
        graph.add_edge(
            "config_gate_range_router",
            "sink_mid",
            label="false",
        ...
        # Set up graph internal maps
        graph._sink_id_map = {
            "too_low": "sink_too_low",
            "mid": "sink_mid",
            "high": "sink_high",
        }
        graph._transform_id_map = {0: "transform_0", 1: "transform_1"}
        graph._config_gate_id_map = {"range_router": "config_gate_range_router"}
        graph._route_resolution_map = {
            # Plugin gate routes
            ("transform_1", "below"): "too_low",  # Route to sink
            ("transform_1", "continue"): "continue",  # Continue to next stage
            # Config gate routes
            ("config_gate_range_router", "true"): "high",
            ("config_gate_range_router", "false"): "mid",
        }
        graph._output_sink = "mid"  # Default output (not used due to gates)
        ...
        # === Verify results ===
        assert result.status == "completed", f"Run failed: {result.status}"
        assert result.rows_processed == 6, "Should process all 6 rows"

        # Verify too_low sink (values < 10: 5, 8)
        assert (
            len(too_low_sink.results) == 2
        ), f"Expected 2 rows in too_low sink, got {len(too_low_sink.results)}"
        too_low_values = {r["value"] for r in too_low_sink.results}
        assert too_low_values == {5, 8}, f"Unexpected too_low values: {too_low_values}"

        # Verify enrichment on too_low rows
        for row in too_low_sink.results:
            assert "doubled" in row, "Row should have 'doubled' field from transform"
            assert row["doubled"] == row["value"] * 2
            assert "label" in row, "Row should have 'label' field from transform"
            assert row["label"].startswith("processed_")

        # Verify mid sink (values 10-30: 15, 25)
        assert (
            len(mid_sink.results) == 2
        ), f"Expected 2 rows in mid sink, got {len(mid_sink.results)}"
        mid_values = {r["value"] for r in mid_sink.results}
        assert mid_values == {15, 25}, f"Unexpected mid values: {mid_values}"

        # Verify enrichment on mid rows
        for row in mid_sink.results:
            assert "doubled" in row, "Row should have 'doubled' field from transform"
            assert row["doubled"] == row["value"] * 2
            assert "label" in row, "Row should have 'label' field from transform"

        # Verify high sink (values > 30: 35, 50)
        assert (
            len(high_sink.results) == 2
        ), f"Expected 2 rows in high sink, got {len(high_sink.results)}"
        high_values = {r["value"] for r in high_sink.results}
        assert high_values == {35, 50}, f"Unexpected high values: {high_values}"

        # Verify enrichment on high rows
        for row in high_sink.results:
            assert "doubled" in row, "Row should have 'doubled' field from transform"
            assert row["doubled"] == row["value"] * 2
            assert "label" in row, "Row should have 'label' field from transform"

        # Verify routing counts
        # - 2 routed by plugin gate (too_low)
        # - 4 pass through plugin gate to config gate
        # - 2 routed by config gate to "high"
        # - 2 routed by config gate to "mid"
        # Total routed = 2 (plugin_gate) + 4 (config_gate)
        # But config_gate routes to named sinks, so all 4 are "routed"
        # Actually, "mid" sink is the default output in this test, so "false" route
        # still counts as routed since it explicitly goes to a named sink
        assert (
            result.rows_routed >= 2
        ), f"Expected at least 2 rows routed by plugin gate, got {result.rows_routed}"
