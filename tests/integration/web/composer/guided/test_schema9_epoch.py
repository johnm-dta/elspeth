...
def test_epoch_34_session_store_fails_before_schema_use(tmp_path: Path) -> None:
    path = tmp_path / "epoch-34.db"
    engine = create_session_engine(f"sqlite:///{path}")
    initialize_session_schema(engine)
    with engine.begin() as connection:
        connection.execute(text("UPDATE elspeth_schema_identity SET schema_epoch = 34 WHERE store_kind = 'session'"))
        connection.execute(text("PRAGMA user_version = 34"))

    with pytest.raises(SessionSchemaError, match=r"SESSION_SCHEMA_EPOCH=35.*Delete the session DB file and restart"):
        initialize_session_schema(engine)
