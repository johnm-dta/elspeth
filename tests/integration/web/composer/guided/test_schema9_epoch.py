...
def test_guided_schema9_admission_barrier_runs_inside_session_epoch_33() -> None:
...
def test_epoch_30_session_store_fails_before_schema_use(tmp_path: Path) -> None:
    ...
    with pytest.raises(SessionSchemaError, match=r"SESSION_SCHEMA_EPOCH=33.*Delete the session DB file and restart"):
        initialize_session_schema(engine)
