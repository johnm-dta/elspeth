...
def db_path(tmp_path: Path) -> Path:
    ...
    rows = [
        # User opens
        ("u1", "S1", "user", "please build me a pipeline", None, None, "2026-05-06 00:00:00"),
        ...
        # Standalone llm_call_audit envelope row (must NOT contribute tool_name)
        (
            "t2",
            "S1",
            "tool",
        ...
        ("a1", "S1", "assistant", "I'm stuck.", None, None, "2026-05-06 00:00:10"),
    ...
    conn.executemany(
        "INSERT INTO chat_messages (id, session_id, role, content, raw_content, tool_calls, created_at) VALUES (?, ?, ?, ?, ?, ?, ?)",
        rows,
    )
    conn.commit()
    conn.close()
...
def test_decode_tool_sequence_returns_ordered_rows_for_session(db_path: Path) -> None:
    ...
    assert [entry["role"] for entry in sequence] == ["user", "tool", "tool", "assistant"]
...
def test_llm_call_audit_envelope_yields_no_tool_name(db_path: Path) -> None:
    ...
    assert llm_audit_row["role"] == "tool"
...
def test_result_summary_truncates_above_300_chars(tmp_path: Path) -> None:
    ...
    conn.execute(
        "INSERT INTO chat_messages (id, session_id, role, content, raw_content, tool_calls, created_at) VALUES (?, ?, ?, ?, ?, ?, ?)",
        ("t1", "S1", "tool", "{}", None, envelope, "2026-05-06 00:00:00"),
    )
    conn.commit()
    conn.close()
