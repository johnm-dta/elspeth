...
class TestBlobTools:
    ...
    def test_create_blob_cleans_file_on_db_failure(self, tmp_path: Path) -> None:
        ...
        with (
            patch(
                "elspeth.web.composer.tools._legacy._check_blob_quota",
...
class TestUpdateBlobQuota:
    ...
    def test_update_exceeding_quota_rejected(self) -> None:
        ...
        with patch("elspeth.web.composer.tools._legacy._BLOB_QUOTA_BYTES", 10):
            result = execute_tool(
                "update_blob",
    ...
    def test_delta_boundary_case(self) -> None:
        ...
        with patch("elspeth.web.composer.tools._legacy._BLOB_QUOTA_BYTES", 500):
            result = execute_tool(
                "update_blob",
    ...
    def test_shrink_on_at_quota_session_succeeds(self) -> None:
        ...
        with patch("elspeth.web.composer.tools._legacy._BLOB_QUOTA_BYTES", len(self.original_content)):
            result = execute_tool(
                "update_blob",
    ...
    def test_quota_delta_uses_current_db_size_not_stale_snapshot(self) -> None:
        ...
        with (
            patch("elspeth.web.composer.tools._legacy._sync_get_blob", side_effect=_get_then_concurrent_write),
            patch("elspeth.web.composer.tools._legacy._BLOB_QUOTA_BYTES", 70),
        ):
            result = execute_tool(
                "update_blob",
...
class TestUpdateBlobRollbackPreservesPrimaryException:
    ...
    def test_primary_db_exception_before_replace_propagates_without_rollback(self) -> None:
        ...
        * no stale tempfile in the storage directory.
        ...
        with (
            patch("elspeth.web.composer.tools._legacy._check_blob_quota", side_effect=_raise_primary),
...
class TestUpdateBlobQuotaRollbackDivergence:
    ...
    def test_quota_breach_returns_failure_without_touching_storage(self) -> None:
        ...
        with (
            patch("elspeth.web.composer.tools._legacy._BLOB_QUOTA_BYTES", 10),
            patch.object(Path, "write_bytes", _tripwire_write_bytes),
        ):
            result = execute_tool(
                "update_blob",
